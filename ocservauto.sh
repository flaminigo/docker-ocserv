...
crl_number = 1 
_EOF_
    certtool --generate-crl --load-ca-privkey ca-key.pem --load-ca-certificate ca-cert.pem --template crl.tmpl --outfile /etc/ocserv/crl.pem
    }
}
...
function check_ca_cert(){
    [ ! -f /usr/local/sbin/ocserv ] && die "Ocserv NOT Found !!!"
...
#input info
    ...
    if [ ! -f /etc/ocserv/$revoke_ca/$revoke_ca-cert.pem ]
    then
...
getuserca | gc)
    character_Test ${LOC_OC_CONF} 'auth = "plain' && {
        character_Test ${LOC_OC_CONF} 'enable-auth = "certificate' || {
            die "You have to enable the certificate login at first."
        }
    }
    get_new_userca
    get_new_userca_show
